function triggerExport(index) {
    if ($.fn.DataTable.isDataTable('#State')) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.exportTable = $('#State').DataTable({
        dom: 'Bfrtip',
        buttons: [
...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/State/Add/${window.selectedClassId}`;
        }
...
async function deleteSelected() {
    ...
    Swal.fire({
        title: 'Are you sure?',
        text: "You are about to delete this State record. This action cannot be undone!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function (res) {
                    if (res.success) {
                        Swal.fire('Deleted!', 'State record has been deleted.', 'success')
                            .then(() => location.reload());
                    } else {
                        Swal.fire('Error!', res.message || 'Failed to delete State.', 'error');
                    }
                },
                error: function () {
...
async function saveItem() {
    ...
    const countyId = document.getElementById('ddlCountry').value.trim();
    if (!countyId) {
        IV.setFieldError('ddlCountry', 'errddlCountry', 'Country name is required.');
        return;
    }

    const name = document.getElementById('txtStateName').value.trim();
    if (!name) {
        IV.setFieldError('txtStateName', 'errtxtStateName', 'Country name is required.');
        return;
    }
    ...
    try {
        ...
        const r = await fetch('/State/SaveState', {
            method: 'POST',
