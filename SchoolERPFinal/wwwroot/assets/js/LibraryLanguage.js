...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/LibraryLanguage/Add/${window.selectedClassId}`;
        }
...
async function deleteSelected() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/LibraryLanguage/DeleteLibraryLanguage',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(selectedIds),
                success: function (res) {
                    if (res.success) {
                        Swal.fire('Deleted!', 'Language record has been deleted.', 'success')
                            .then(() => location.reload());
                    } else {
                        Swal.fire('Error!', res.message || 'Failed to delete language.', 'error');
                    }
                },
                error: function () {
...
async function saveItem() {
    ...
    const name = document.getElementById('txtLanguageName').value.trim();
    if (!name) {
        IV.setFieldError('txtLanguageName', 'errtxtLanguageName', 'Language name is required.');
        return;
    }
    ...
    try {
        ...
        const r = await fetch('/LibraryLanguage/SaveLibraryLanguage', {
            method: 'POST',
        ...
        if (res.success) {
            Swal.fire({
                icon: 'success',
                title: 'Saved!',
                text: res.message || 'Language has been added successfully.',
                confirmButtonText: 'OK',
                customClass: { confirmButton: 'btn btn-success' },
                buttonsStyling: false
            }).then(() => {
                window.location.href = '/LibraryLanguage/Index';
            });
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: res.message || 'Failed to save language.',
                confirmButtonText: 'OK',
        ...
        console.error(err);
        IV.setNotice('classModalNotice', 'Could not save publisher.');
    }
}
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    url: '/LibraryLanguage/ToggleLibraryLanguage',
                    type: 'POST',
                    contentType: 'application/json',
