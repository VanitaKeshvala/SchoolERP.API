...
// ------------------------------------------------------------------
// LOAD EXISTING MASTER + CHAPTERS WHEN CLASS/SECTION/GROUP/SUBJECT
// ARE ALL SELECTED (EDIT PATH). IF NO MASTER EXISTS YET FOR THIS
// COMBO, hdnLessonId STAYS 0 AND THE SP WILL CREATE ONE ON SAVE.
// ------------------------------------------------------------------
function loadExistingLessons() {
    ...
    $.get('/Homework/GetLessonsBySubject', {
        classId: classId,
        sectionId: sectionId,
        subjectGroupId: subjectGroupId,
        subjectId: subjectId
    }, function (r) {
        const container = $('#lessonRowsContainer');
        container.empty();

        // Expected shape: { success, data: { lessonId, chapters: [{ lessonMapId, lessonName, description }] } }
        const lessonId = (r.success && r.data && r.data.lessonId) ? r.data.lessonId : 0;
        $('#hdnLessonId').val(lessonId);

        const chapters = (r.success && r.data && r.data.chapters) ? r.data.chapters : [];

        if (chapters.length > 0) {
            chapters.forEach(item => {
                addNewLessonRow();
                const $row = container.children('.lesson-row').last();
                $row.attr('data-lesson-map-id', item.lessonMapId);
                $row.find('.lesson-map-id').val(item.lessonMapId);
                $row.find('.lesson-name').val(item.lessonName);
                $row.find('.lesson-description').val(item.description);
            });
        } else {
            addNewLessonRow();
        }
    }).fail(function () {
        showToast('Unable to load existing lessons', 'error');
    });
}
...
async function saveAllLessons() {
    ...
    const subjectGroupId = $('#ddlSubjectGroup').val();
    const subjectId = $('#ddlSubject').val();
    const lessonId = parseInt($('#hdnLessonId').val()) || 0;
    ...
    formData.append('LessonJson', JSON.stringify(lessonChapters));

    
    try {
        const res = await fetch('/LessonPlan/UpsertLessonPlan', {
            method: 'POST',
            body: formData
        }).then(r => r.json());
        ...
        if (res.success) {
            ...
            }).then(() => window.location.href = '/LessonPlan/Index');
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Error',
...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/LessonPlan/Add/${window.selectedClassId}`;
        }
...
async function deleteSelected() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/LessonPlan/DeleteLessonPlan',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(selectedIds),
                success: function (res) {
                    if (res.success) {
                        Swal.fire('Deleted!', 'Lesson plan record has been deleted.', 'success')
                            .then(() => location.reload());
                    } else {
                        Swal.fire('Error!', res.message || 'Failed to delete Lesson plan.', 'error');
                    }
                },
                error: function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    url: '/LessonPlan/ToggleLessonPlankStatus',
                    type: 'POST',
                    contentType: 'application/json',
