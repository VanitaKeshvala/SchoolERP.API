...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedPurposesId) {
            location.href = `/Source/AddSource/${window.selectedPurposesId}`;
        }
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function (res) {
                    if (res.success) {
                        Swal.fire('Deleted!', 'Source record has been deleted.', 'success')
                            .then(() => location.reload());
                    } else {
                        Swal.fire('Error!', res.message || 'Failed to delete source.', 'error');
                    }
                },
                error: function () {
...
async function saveRecord() {
    ...
    const r = await fetch('/Source/SaveSource', {
        method: 'POST',
    ...
    if (res.success) {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: res.message || 'Failed to save source.',
            confirmButtonText: 'OK',
