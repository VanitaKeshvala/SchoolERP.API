...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.exportTable = $('#tblCountry').DataTable({
        dom: 'Bfrtip',
        buttons: [
...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/Country/Add/${window.selectedClassId}`;
        }
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function (res) {
                    if (res.success) {
                        Swal.fire('Deleted!', 'Country record has been deleted.', 'success')
                            .then(() => location.reload());
                    } else {
                        Swal.fire('Error!', res.message || 'Failed to delete Country.', 'error');
                    }
                },
                error: function () {
...
async function saveItem() {
    ...
    const name = document.getElementById('txtCountryName').value.trim();
    if (!name) {
        IV.setFieldError('txtCountryName', 'txtCountryName', 'Country name is required.');
        return;
    }
    ...
    try {
        ...
        const r = await fetch('/Country/SaveCountry', {
            method: 'POST',
