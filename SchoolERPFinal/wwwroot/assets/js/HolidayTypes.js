...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/HolidayType/Add/${window.selectedClassId}`;
        }
...
async function deleteSelected() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/HolidayType/DeleteHolidayType',
                type: 'POST',
...
async function saveItem() {
    ...
    const name = document.getElementById('txtHolidayTypeName').value.trim();
    if (!name) {
        IV.setFieldError('txtHolidayTypeName', 'errtxtHolidayTypeName', 'Hostel Type name is required.');
        return;
    }

    const colourCode = $('#txtColourCode').val();
    ...
    try {
        ...
        const r = await fetch('/HolidayType/SaveHolidayType', {
            method: 'POST',
        ...
        if (res.success) {
            Swal.fire({
                ...
                window.location.href = '/HolidayType/Index';
            });
        } else {
            Swal.fire({
                icon: 'error',
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    url: '/HolidayType/ToggleHolidayType',
                    type: 'POST',
                    contentType: 'application/json',
