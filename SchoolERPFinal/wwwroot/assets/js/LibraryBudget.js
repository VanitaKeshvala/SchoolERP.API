...
// ========================================
const FILTER_KEY = 'appliedFilters_budget';
let appliedFilters = {};
try {
...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/LibraryBudget/Add/${window.selectedClassId}`;
        }
...
async function deleteSelected() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/LibraryBudget/DeleteLibraryBudge',
                type: 'POST',
                ...
                success: function (res) {
                    if (res.success) {
                        ...
                        Swal.fire('Error!', res.message || 'Failed to delete budget.', 'error');
                    }
                },
                error: function () {
...
async function saveItem() {
    ...
    const name = document.getElementById('txtBudgetName').value.trim();
    if (!name) {
        IV.setFieldError('txtBudgetName', 'errtxtBudgetName', 'Budget name is required.');
        return;
    }

    const acronym = document.getElementById('txtAcronym').value.trim();
    if (!acronym) {
        IV.setFieldError('txtAcronym', 'errtxtAcronym', 'Acronym is required.');
        return;
    }
    ...
    try {
        ...
        const r = await fetch('/LibraryBudget/SaveLibraryBudget', {
            method: 'POST',
        ...
        if (res.success) {
            Swal.fire({
                icon: 'success',
                title: 'Saved!',
                text: res.message || 'budget has been added successfully.',
                confirmButtonText: 'OK',
                customClass: { confirmButton: 'btn btn-success' },
                buttonsStyling: false
            }).then(() => {
                window.location.href = '/LibraryBudget/Index';
            });
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: res.message || 'Failed to save budget.',
                confirmButtonText: 'OK',
        ...
        console.error(err);
        IV.setNotice('classModalNotice', 'Could not save budget.');
    }
}
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    url: '/LibraryBudget/ToggleLibraryBudge',
                    type: 'POST',
                    contentType: 'application/json',
