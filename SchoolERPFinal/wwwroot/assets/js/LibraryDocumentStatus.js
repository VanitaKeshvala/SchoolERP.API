...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/LibraryDocumentStatus/Add/${window.selectedClassId}`;
        }
...
async function deleteSelected() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/LibraryDocumentStatus/DeleteLibraryDocumentStatus',
                type: 'POST',
...
async function saveItem() {
    ...
    const name = document.getElementById('txtStatusName').value.trim();
    if (!name) {
        IV.setFieldError('txtStatusName', 'errtxtStatusName', 'Status Name is required.');
        return;
    }

    const statusCode = document.getElementById('txtStatusCode').value.trim();
    if (!statusCode) {
        IV.setFieldError('txtStatusCode', 'errtxtStatusCode', 'Status Code is required.');
        return;
    }

    // ✅ Fix 3: isActive null guard
    var canBeIssued = $("#chkCanBeIssued").is(":checked");
    ...
    try {
        ...
        const r = await fetch('/LibraryDocumentStatus/SaveLibraryDocumentStatus', {
            method: 'POST',
        ...
        if (res.success) {
            Swal.fire({
                icon: 'success',
                title: 'Saved!',
                text: res.message || 'Status has been added successfully.',
                confirmButtonText: 'OK',
                customClass: { confirmButton: 'btn btn-success' },
                buttonsStyling: false
            }).then(() => {
                window.location.href = '/LibraryDocumentStatus/Index';
            });
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: res.message || 'Failed to save status.',
                confirmButtonText: 'OK',
        ...
        console.error(err);
        IV.setNotice('classModalNotice', 'Could not save status.');
    }
}
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    url: '/LibraryDocumentStatus/ToggleLibraryDocumentStatus',
                    type: 'POST',
                    contentType: 'application/json',
