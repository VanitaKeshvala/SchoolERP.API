...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/Author/Add/${window.selectedClassId}`;
        }
...
async function saveItem() {
    ...
    const name = document.getElementById('txtAuthorName').value.trim();
    if (!name) {
        IV.setFieldError('txtAuthorName', 'errtxtAuthorName', 'Author name is required.');
        return;
    }
    ...
    try {
        ...
        const r = await fetch('/Author/SaveAuthor', {
            method: 'POST',
        ...
        if (res.success) {
            Swal.fire({
                icon: 'success',
                title: 'Saved!',
                text: res.message || 'Author has been added successfully.',
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: res.message || 'Failed to save author.',
                confirmButtonText: 'OK',
        ...
        console.error(err);
        IV.setNotice('classModalNotice', 'Could not save author.');
    }
}
