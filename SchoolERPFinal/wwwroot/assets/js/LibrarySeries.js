...
// ========================================
const FILTER_KEY = 'appliedFilters_librarySeries';
let appliedFilters = {};
try {
...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/LibrarySeries/Add/${window.selectedClassId}`;
        }
...
async function deleteSelected() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/LibrarySeries/DeleteLibrarySeries',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(selectedIds),
                success: function (res) {
                    if (res.success) {
                        Swal.fire('Deleted!', 'Series record has been deleted.', 'success')
                            .then(() => location.reload());
                    } else {
                        Swal.fire('Error!', res.message || 'Failed to delete series.', 'error');
                    }
                },
                error: function () {
...
async function saveItem() {
    ...
    const name = document.getElementById('txtSeriesName').value.trim();
    if (!name) {
        IV.setFieldError('txtSeriesName', 'errtxtSeriesName', 'Series name is required.');
        return;
    }

    const seriesCode = document.getElementById('txtSeriesCode').value.trim();
    ...
    try {
        ...
        const r = await fetch('/LibrarySeries/SaveLibrarySeries', {
            method: 'POST',
        ...
        if (res.success) {
            Swal.fire({
                icon: 'success',
                title: 'Saved!',
                text: res.message || 'Series has been added successfully.',
                confirmButtonText: 'OK',
                customClass: { confirmButton: 'btn btn-success' },
                buttonsStyling: false
            }).then(() => {
                window.location.href = '/LibrarySeries/Index';
            });
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: res.message || 'Failed to save series.',
                confirmButtonText: 'OK',
        ...
        console.error(err);
        IV.setNotice('classModalNotice', 'Could not save series.');
    }
}
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    url: '/LibrarySeries/ToggleLibrarySeries',
                    type: 'POST',
                    contentType: 'application/json',
