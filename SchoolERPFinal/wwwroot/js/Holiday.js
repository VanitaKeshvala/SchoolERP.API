...
async function editSelected() {
    ...
    try {
        ...
        if (window.selectedClassId) {
            location.href = `/Holiday/Add/${window.selectedClassId}`;
        }
...
async function saveItem() {
    ...
    const name = document.getElementById('txtHolidayName').value.trim();
    if (!name) {
        IV.setFieldError('txtHolidayName', 'errtxtHolidayName', 'Hostel Type name is required.');
        return;
    }

    const holidayType = $('#ddlHolidayType').val();
    ...
    const data = {
        ...
        holidayName: name,
        fromDate: document.getElementById('txtFromDate').value.trim() || null, 
        toDate: document.getElementById('txtToDate').value.trim() || null, 
        isActive: isActive
    };

    try {

        const r = await fetch('/Holiday/SaveHoliday', {
            method: 'POST',
