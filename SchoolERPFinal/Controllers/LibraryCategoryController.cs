...
namespace SchoolERP.Net.Controllers
{
    ...
    {
        private readonly ILibraryCategoryClientService _client;
        private readonly ICompanyClientService _companyService;
        private readonly ISessionClientService _sessionService;

        public LibraryCategoryController(ILibraryCategoryClientService client,
        ...
        int? companyId)
        {
            try
            {
                ...
                var budgetResponse = _client.GetAllLibraryCategoryWithPagePageAsync(request);
                ...
                var model = new LibraryCategoryPageViewModel
                {
                    Category = pagedResult.Success ? pagedResult.Data.Data : new List<LibraryCategoryModel>(),
        ...
        public async Task<IActionResult> Add(int? id)
        {
            try
            {
                ...
                var model = new LibraryCategoryAddViewModel();                
                if (id.HasValue && id.Value > 0)
                {
                    ...
                    if (response.Success)
                    {
                        ...
                        model.EditCategory = response.Data;
                    }
                }
                else
                {
                    model.EditCategory = null;
                }
        ...
        [HttpPost]
        public async Task<IActionResult> SaveLibraryCategory([FromBody] LibraryCategoryRequest request)
        {
            try
            {
                var isCreate = request.CompanyID <= 0;
                request.CompanyID = await GetCompanyId();
                var response = await _client.UpsertLibraryCategoryAsync(request);
