...
namespace SchoolERP.API.Services
{
    public class HolidayService: IHolidayService
    {
        ...
        // ============================================================
        public async Task<ApiResponse> UpsertHolidayAsync(HolidayRequestModel model)
        {
        ...
        public async Task<HolidayModel?> GetHolidayByIdAsync(int HolidayId)
        {
            try
            {
                ...
                return await conn.QueryFirstOrDefaultAsync<HolidayModel>(
                    "SP_TBL_MST_HOLIDAY_GETBYID",
                    parameters,
                    commandType: CommandType.StoredProcedure);
            }
            catch (Exception ex)
        ...
        // ============================================================
        public async Task<List<HolidayModel>> GetAllHolidayAsync(int companyId, int sessionId, bool includeDeleted = false)
        {
            try
            {
                using var conn = new SqlConnection(
                ...
                var result = conn.Query<HolidayModel>(
                    "SP_TBL_MST_HOLIDAY_GETALL",
                    parameters,
                    commandType: CommandType.StoredProcedure
                ).ToList();
        ...
        public async Task<PagedResult<HolidayModel>> GetAllHolidayWithPage(HostelTypeSearchRequest req)
        {
            try
            {
                ...
                var result = (await conn.QueryAsync<HolidayModel>(
                "SP_TBL_MST_HOLIDAY_GETALLWITHPAGEINDEX",
                param,
                ...
                var userModel = new PagedResult<HolidayModel>
                {
                    Data = result,
                ...
                if (res == 0)
                {
                    userModel = new PagedResult<HolidayModel>
                    {
                        Data = null,
                        TotalRecords = totalRecords,
                        PageNumber = pageIndex,
        ...
        public (bool success, string message) DeleteHoliday(List<int> ids, int userId)
        {
            try
            {
                if (ids == null || !ids.Any())
                ...
                var parameters = new DynamicParameters();
                parameters.Add("@HOLIDAYID", hostelTypeID);
                parameters.Add("@USERID", userId);
                var result = conn.QueryFirstOrDefault<SpResult>(
                   "SP_MST_HOLIDAY_DELETE",
                   parameters,
                   commandType: CommandType.StoredProcedure);
        ...
        /// <param name="userId">Logged-in user ID.</param>
        /// <returns>Operation status and message.</returns>
        public (bool success, string message) ToggleHolidayStatus(StatusUpdateRequest request)
        {
            try
            {
                using var conn = new SqlConnection(
                ...
                var parameters = new DynamicParameters();
                parameters.Add("@HOLIDAYID", request.Ids);
                parameters.Add("@IsActive", request.IsActive);
                parameters.Add("@UserId", request.DoneBy);

                var result = conn.QueryFirstOrDefault<SpResult>(
                    "SP_MST_HOLIDAY_TOGGLESTATUS",
                    parameters,
                    commandType: CommandType.StoredProcedure);
        ...
        public async Task<(bool Success, string Message)> CopyHolidayToSession(CopyRequest req)
        {
            try
            {
                using var conn = new SqlConnection(
